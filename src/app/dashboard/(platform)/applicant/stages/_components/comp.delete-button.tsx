...
import { deleteStage } from "../_actions/action.stage";
...
export function DeleteButton({ id, name }: Props) {
  ...
  async function handleDelete() {
    ...
    if (result?.error) {
      ...
      setOpen(false);
      setTimeout(() => {
        toast.success("Stage berhasil dihapus", { position: "top-right" });
      }, 150);
    }
  }
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Apakah Anda yakin ingin menghapus stage <strong>{name}</strong>? Tindakan ini tidak dapat dibatalkan.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
