...
import { deleteExperience } from "../_actions/action.experience";
...
export function DeleteButton({ id, name }: Props) {
  ...
  async function handleDelete() {
    ...
    if (result?.error) {
      ...
      router.refresh();
      setTimeout(() => {
        toast.success("Pengalaman berhasil dihapus", { position: "top-right" });
      }, 150);
    }
  }
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Apakah Anda yakin ingin menghapus pengalaman <strong>{name}</strong>? Tindakan ini tidak dapat dibatalkan.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
