...
import { deleteEducation } from "../_actions/action.education";
...
export function DeleteButton({ id, name }: Props) {
  ...
  async function handleDelete() {
    ...
    if (result?.error) {
      ...
      router.refresh();
      setTimeout(() => {
        toast.success("Pendidikan berhasil dihapus", { position: "top-right" });
      }, 150);
    }
  }
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Apakah Anda yakin ingin menghapus pendidikan <strong>{name}</strong>? Tindakan ini tidak dapat dibatalkan.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
