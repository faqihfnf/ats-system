...
import { deleteJob } from "../_actions/action.job";
...
export function DeleteButton({ id, name }: Props) {
  ...
  async function handleDelete() {
    ...
    if (result?.error) {
      ...
      router.refresh();
      setTimeout(() => {
        toast.success("Lowongan berhasil dihapus", { position: "top-right" });
      }, 150);
    }
  }
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Apakah Anda yakin ingin menghapus lowongan <strong>{name}</strong>? Tindakan ini tidak dapat dibatalkan.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
