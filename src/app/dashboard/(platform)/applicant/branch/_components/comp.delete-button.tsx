...
import { deleteBranch } from "../_actions/action.branch";
...
export function DeleteButton({ id, name }: Props) {
  ...
  async function handleDelete() {
    ...
    if (result?.error) {
      ...
      router.refresh();
      setTimeout(() => {
        toast.success("Branch berhasil dihapus", { position: "top-right" });
      }, 150);
    }
  }
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Apakah Anda yakin ingin menghapus branch <strong>{name}</strong>? Tindakan ini tidak dapat dibatalkan.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
