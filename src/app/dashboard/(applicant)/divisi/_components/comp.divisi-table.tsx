...
import { DivisiForm } from "./comp.divisi-form";
import { DeleteButton } from "./comp.delete-button";

type DivisiWithCount = {
  id: string;
  nama: string;
  _count: { positions: number };
};

export function DivisiTable({ data }: { data: DivisiWithCount[] }) {
  if (data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-12 text-center">
        <p className="text-sm text-muted-foreground">Belum ada divisi. Tambahkan divisi pertama Anda.</p>
      </div>
    );
  }
          ...
          {data.map((item) => (
            <TableRow key={item.id}>
              ...
              <TableCell className="text-center">
                <DivisiForm divisi={{ id: item.id, nama: item.nama }} />
              </TableCell>
              <TableCell className="text-center">
                <DeleteButton
                  id={item.id}
